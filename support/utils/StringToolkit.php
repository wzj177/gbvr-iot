...
class StringToolkit
{
    ...
    public static function getPasswordEncoder($algo = 'sha256')
    {
        return new MessageDigestPasswordHasher($algo);
    }
    ...
    public static function chinese_to_hex($chinese)
    {
        $utf8String = mb_convert_encoding($chinese, 'UTF-8', 'auto');
        $hexString = bin2hex($utf8String);

        return $hexString;

    }

    public static function is_valid_date($dateString, $format)
    {
     ...
     * @param int $level 压缩等级， 范围 0 ~ 9， 最低为0，不压缩，最高为9，最消耗cpu资源， 默认为5
     */
