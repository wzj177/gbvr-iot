...
class DeviceToolkit
{
    ...
    public static function isPcClient($userAgent)
    {
    ...
    public static function isMobileClient()
    {
        ...
        if (isset($header['user_agent'])) {
            ...
            // 从USER_AGENT中查找手机浏览器的关键字
            if (preg_match('/(' . implode('|', $clientkeywords) . ')/i', strtolower($header['user_agent']))) {
                return true;
            }
        }
        ...
        if (isset($header['accept'])) {
            ...
            if ((false !== strpos($header['accept'], 'vnd.wap.wml')) && (false === strpos($header['accept'], 'text/html') || (strpos($header['accept'], 'vnd.wap.wml') < strpos($header['accept'], 'text/html')))) {
                return true;
            }
        }
    ...
    private static function matchBrowser()
    {
        ...
        $agent = $header['user_agent'];  //获取用户代理字符串
