...
class FileToolkit
{
    public static function removeDirFiles($path, $ext = 'jpg')
    {
     ...
     * @return array
     */
    public static function getDirs($dirname)
    {
        $files = [];
        if (is_dir($dirname)) {
            $fileHandler = opendir($dirname);
     ...
     * @return array
     */
    public static function getFiles($dirname, $deep = false)
    {
        $files = [];
        if (is_dir($dirname)) {
            $fileHandler = opendir($dirname);
            while (($file = readdir($fileHandler)) !== false) {
                ...
                if (is_dir($filepath)) {
                    ...
                    $files[] = self::getFiles($filepath, $deep);
                }
                $files[] = self::autoCharset($file, 'GBK', 'UTF8');
            }
    ...
    public static function getFileTypeByExtension($extension)
    {
        ...
        if (in_array($extension, ['vob', 'mp4', 'avi', 'flv', 'f4v', 'wmv', 'mov', 'rmvb', 'mkv', 'mpg', 'm4v', 'webm', 'rm', 'mpeg', 'asf', 'ts', 'mts'])) {
            return 'video';
        } else if (in_array($extension, ['mp3', 'wma', 'wav'])) {
            return 'audio';
        } else if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'bmp'])) {
            return 'image';
        } else if (in_array($extension, ['doc', 'docx', 'pdf', 'xls', 'xlsx', 'txt'])) {
            return 'document';
        } else if (in_array($extension, ['ppt', 'pptx'])) {
            return 'ppt';
        } else if (in_array($extension, ['swf'])) {
            return 'flash';
        } else if (in_array($extension, ['srt'])) {
            return 'subtitle';
        } else {
            return 'other';
        }
    ...
    public static function getDownloadTaskTypeByExtension($extension)
    {
        ...
        if (in_array($extension, ['mpeg', 'mpg', 'mpe', 'mlv', 'dat', '2v', 'vob', 'rmvb', 'mov', 'qt', 'asf', 'avi', 'wmv', 'mkv', 'mp4', 'flv'])) {
            return 'video';
        } else if (in_array($extension, ['mp3', 'wma', 'aac', 'cda', 'wav', 'voc', 'cda'])) {
            return 'audio';
        } else if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif'])) {
            return 'image';
        } else if (in_array($extension, ['zip', 'zipx', 'rar', '7z', 'dmg', 'tar'])) {
            return 'package';
            ...
            return 'ppt';
        } else if (in_array($extension, ['flash'])) {
            return 'flash';
        } else if (in_array($extension, ['link'])) {
            return 'link';
        } else {
            return 'other';
        }
