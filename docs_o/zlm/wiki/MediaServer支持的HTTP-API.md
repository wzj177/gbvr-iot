...
由于接口更新频繁，在观看本文档的同时，请大家再使用postman核对测试接口，同时本文档可能有些接口有遗漏，你也可以参考[代码(查看api_regist相关)](https://github.com/xia-chu/ZLMediaKit/blob/master/server/WebApi.cpp)
...
### 19、`/index/api/getMp4RecordFile`
  ...
  - 参数：
    ...
    |  app   |    Y     |                          流的应用名                          |
    | stream |    Y     |                            流的ID                            |
    | period |    N     | 流的录像日期，格式为2020-02-01,如果不是完整的日期，那么是搜索录像文件夹列表，否则搜索对应日期下的mp4文件列表 |
...
- **参数**:
  ...
  | id     | 拼接任务的唯一标识 | 该示例中为`stack_test`，则取流url为：`rtsp://ip/live/stack_test`  (其他类型的流取流规则与常规流相同) |
  | row    | 宫格的行数     |                                                                           |
  | col    | 宫格的列数     |                                                                           |
...
![videostack](https://github.com/ZLMediaKit/ZLMediaKit/assets/44298896/b568aa9b-cb4c-4267-a321-6d93185bcb40)
