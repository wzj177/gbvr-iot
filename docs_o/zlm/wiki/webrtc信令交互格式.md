...
|  app   | Y | string |流应用名|
| stream | Y | string |流名称|
| type | Y | string |push/play/echo|
| preferred_tcp | N | bool/int |是否为tcp模式优先，默认否|
| cand_tcp | N | string |ip:port, zlm对播放器可见webrtc tcp地址|
| cand_udp | N | string |ip:port, zlm对播放器可见webrtc  udp地址|
