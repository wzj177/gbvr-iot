...
class GBS
{
    ...
    private function processMessageQueue() : void
    {
        while ($this->running) {
            ...
            [$message, $addr] = $item;
            // 限流：对应 Python ThreadPoolExecutor(max_workers=50)
    ...
    private function handleMessageRequest(string $message, array $addr) : void
    {
        try {
            ...
            if (str_contains($body, 'Catalog')) {
                ...
                $channelTypes = [];
                try {
                    $root = Helper::gb_safe_xml_parse($body);
                    $itemList = Helper::gb_xml_find_all($root, 'Item');
                    if ($itemList) {
                        $channelCount = count($itemList);
                        foreach ($itemList as $item) {
                            $chIdElem = Helper::gb_xml_child($item, 'DeviceID');
                            $parentalElem = Helper::gb_xml_child($item, 'Parental');
                            if ($chIdElem !== null) {
                                $chId = Helper::gb_xml_text($chIdElem);
                                $parental = $parentalElem !== null && Helper::gb_xml_text($parentalElem) !== '' ? (int)Helper::gb_xml_text($parentalElem) : 0;
                                $channelIds[] = $chId;
            ...
            if (str_contains($body, 'DeviceInfo')) {
                ...
                try {
                    $root = Helper::gb_safe_xml_parse($body);
                    $deviceName = Helper::gb_xml_text(Helper::gb_xml_child($root, 'DeviceName'));
                    $manufacturer = Helper::gb_xml_text(Helper::gb_xml_child($root, 'Manufacturer'));
                    $model = Helper::gb_xml_text(Helper::gb_xml_child($root, 'Model'));
                    $firmware = Helper::gb_xml_text(Helper::gb_xml_child($root, 'FirmwareVersion'));
        ...
        try {
            ...
            try {
                ...
                foreach ($itemList as $item) {
                    ...
                    $name = Helper::gb_xml_text(Helper::gb_xml_child($item, 'Name'));
                    $statusText = Helper::gb_xml_text(Helper::gb_xml_child($item, 'Status'));
                    $status = $statusText !== '' ? $statusText : 'OFF';
                    $parentalText = Helper::gb_xml_text(Helper::gb_xml_child($item, 'Parental'));
                    $parental = $parentalText !== '' ? (int)$parentalText : 0;
                    $parentIdText = Helper::gb_xml_text(Helper::gb_xml_child($item, 'ParentID'));
                    $pid = $parentIdText !== '' ? $parentIdText : ($parentId !== '' ? $parentId : $deviceId);
                    $devType = Helper::gb_xml_text(Helper::gb_xml_child($item, 'DeviceType'));
                    $manufacturer = Helper::gb_xml_text(Helper::gb_xml_child($item, 'Manufacturer'));
                    $model = Helper::gb_xml_text(Helper::gb_xml_child($item, 'Model'));
                    $owner = Helper::gb_xml_text(Helper::gb_xml_child($item, 'Owner'));
                    $civilCode = Helper::gb_xml_text(Helper::gb_xml_child($item, 'CivilCode'));
                    $sumNumText = Helper::gb_xml_text(Helper::gb_xml_child($item, 'SumNum'));
