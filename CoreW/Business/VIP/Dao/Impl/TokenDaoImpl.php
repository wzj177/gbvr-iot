<?php


namespace CoreW\Business\VIP\Dao\Impl;


use CoreW\Business\User\Dao\TokenDao;
use CoreW\Dao\GeneralDaoImpl;

class TokenDaoImpl extends GeneralDaoImpl implements TokenDao
{
    protected $table = 'gv_vip_token';

    public function getByToken($token)
    {
        $sql = "SELECT * FROM {$this->table} WHERE token = ? LIMIT 1";

        return $this->db()->fetchAssoc($sql, [$token]) ?: null;
    }

    public function findByVIPIdAndType($vipId, $type)
    {
        return $this->findByFields(['vipId' => $vipId, 'type' => $type]);
    }

    public function destroyTokensByVIPId($vipId)
    {
        return $this->db()->delete($this->table, ['vipId' => $vipId]);
    }

    public function getByType($type)
    {
        $sql = "SELECT * FROM {$this->table} WHERE type = ?  and expiredTime > ? order  by createdTime DESC  LIMIT 1";

        return $this->db()->fetchAssoc($sql, [$type, time()]) ?: null;
    }

    public function deleteTopsByExpiredTime($expiredTime, int $limit)
    {
        $sql = "DELETE FROM {$this->table} WHERE expiredTime < ? LIMIT {$limit} ";

        return $this->db()->executeQuery($sql, [$expiredTime]);
    }

    public function deleteByTypeAndVIPId($type, $vipId)
    {
        return $this->db()->delete($this->table, ['type' => $type, 'vipId' => $vipId]);
    }

    public function getLastedByVIPIdAndType($vipId, $type)
    {
        $sql = "SELECT * FROM {$this->table} WHERE type = ?  and vipId = ? and expiredTime > ? order  by id DESC  LIMIT 1";

        return $this->db()->fetchAssoc($sql, [$type, $vipId, time()]) ?: null;
    }

    public function declares():array
    {
        return [
            'cache' => false,
            'serializes' => [
                'data' => 'php'
            ],
            'conditions' => [
                'type = :type'
            ],
        ];
    }
}