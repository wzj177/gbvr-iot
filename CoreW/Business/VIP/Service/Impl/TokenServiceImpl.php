...
use CoreW\Business\VIP\Token\Storage\DbTokenStorage;
use CoreW\Business\VIP\Token\Storage\RedisTokenStorage;
...
class TokenServiceImpl extends BaseService implements TokenService
{
    ...
    /**
     *
     * 支持   传递duration：过期时间（单位：s); 传递expired_time：具体时间戳
     * @param string $type
     * @param array $args
     * @return array
     */
    public function makeToken($type, array $args = [])
    {
        ...
        $token['vipId'] = empty($args['vipId']) ? 0 : $args['vipId'];
        if (!isset($args['expired_time'])) {
            $token['expiredTime'] = empty($args['duration']) ? 0 : time() + $args['duration'];
        } else {
            $token['expiredTime'] = (int)$args['expired_time'];
        }
