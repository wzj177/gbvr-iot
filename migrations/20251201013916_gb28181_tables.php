...
CREATE TABLE `gv_devices` (
  ...
  `status` enum('online','expired','unregistered') NOT NULL DEFAULT 'unregistered' COMMENT '设备状态',
  `enabled` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否启用',
  `sum_num` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '设备通道总数',
  `ip` varchar(45) DEFAULT NULL COMMENT '设备IP地址',
  `port` int(11) DEFAULT NULL COMMENT '设备SIP端口',
  `from_uri` varchar(100) NOT NULL DEFAULT '' COMMENT 'SIP From 头',
  `contact` varchar(200) NOT NULL DEFAULT '' COMMENT 'SIP Contact 头',
  `manufacturer` varchar(255) NOT NULL DEFAULT '' COMMENT '设备制造商',
  `model` varchar(100) NOT NULL DEFAULT '' COMMENT '设备型号',
  `firmware` varchar(100) NOT NULL DEFAULT '' COMMENT '固件',
  `user_agent` varchar(200) NOT NULL DEFAULT '' COMMENT '设备固件或 SDK 标识',
  `expires` int(4) NOT NULL DEFAULT '0' COMMENT '注册有效期（秒）',
  `registered_at` datetime DEFAULT NULL COMMENT '注册时间',
  `last_heartbeat_at` int(11) NOT NULL  DEFAULT '0' COMMENT '最后心跳时间',
  ...
  `show_name` varchar(255) NOT NULL DEFAULT '' COMMENT '自定义名称',
  `province_id` varchar(16) NOT NULL DEFAULT '' COMMENT '所在省份',
...
-- GB28181 设备通道表
CREATE TABLE `gv_device_channels` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `device_id` varchar(32) NOT NULL COMMENT '设备国标ID',
  `channel_id` varchar(32) NOT NULL COMMENT '通道国标ID',
  `channel_type` varchar(32) DEFAULT '' COMMENT '通道类型, video=视频,alarm=报警, audio=音频，other=其他',
  `channel_name` varchar(100) DEFAULT '' COMMENT '通道名称',
  `manufacturer` varchar(100) DEFAULT '' COMMENT '设备制造商',
  `owner` varchar(100) NOT NULL DEFAULT '' COMMENT '所属组织/用户',
  `model` varchar(100) NOT NULL DEFAULT '' COMMENT '设备型号',
  `parental` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否为父设备，0 = 叶子节点（视频通道），不可再挂子设备；1 = 可挂子设备（如 NVR、平台',
  `parent_id` varchar(32) DEFAULT '' COMMENT '父级ID,所属上级设备（此处为一台 HCVR 或 NVR）',
  ...
  `custom_lng` varchar(40) NOT NULL DEFAULT '' COMMENT '自填经度（标注或者手填）',
  `is_admin_area_pos` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否是行政区域经纬度信息',
  `main_id` varchar(32) NOT NULL COMMENT 'ZLM流ID，兼容akstream，效果和stream_id一样，device_id_channel_id的组合',
  `ssrc` varchar(10) NOT NULL COMMENT 'SSRC（10位数字）',
  `stream_id` varchar(64) NOT NULL COMMENT 'ZLM流ID',
  ...
  `origin_code` varchar(100) NOT NULL DEFAULT '' COMMENT '级联编号',
  `created_at` datetime NOT NULL COMMENT '创建时间',
  `updated_at` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `gv_stream_sessions` (
  ...
  `dialog_id` varchar(32) DEFAULT '0' COMMENT 'SIP DiaLog-ID',
  `device_id` varchar(32) NOT NULL COMMENT '设备国标ID',
  `channel_id` varchar(32) NOT NULL COMMENT '通道国标ID',
  `ssrc` varchar(10) NOT NULL COMMENT 'SSRC',
  `stream_id` varchar(64) NOT NULL COMMENT '流ID',
  `media_server_id` varchar(64) NOT NULL COMMENT '流媒体ID',
  ...
  `rtp_port` int(11) DEFAULT NULL COMMENT ' RTP端口',
  `tcp_mode` tinyint(2) NOT NULL DEFAULT '0' COMMENT 'RTP传输模式,0=UDP,1=TCP被动（设备连服务器），2=TCP主动（服务器连设备）',
  `start_time` varchar(20) DEFAULT NULL COMMENT '回放开始时间',
  `end_time` varchar(20) DEFAULT NULL COMMENT '回放结束时间',
  `device_ip` varchar(32) DEFAULT NULL COMMENT '设备会话IP',
  `device_port` int(8) NOT NULL DEFAULT '0' COMMENT '设备会话端口',
  `started_at` datetime NOT NULL COMMENT '开始时间',
  `stopped_at` datetime DEFAULT NULL COMMENT '结束时间',
 `sdp` longtext DEFAULT NULL  COMMENT 'sdp信息',
 `viewer_count` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '代理请求次数',
 `player_count` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '观众人数',
`last_reader_time` datetime DEFAULT NULL COMMENT '最后观看时间',
  `created_at` datetime NOT NULL COMMENT '创建时间',
  `updated_at` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
-- 添加索引优化查询性能
ALTER TABLE `gv_devices` ADD INDEX `idx_created_at` (`created_at`);
